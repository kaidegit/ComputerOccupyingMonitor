...
extern unsigned int get_systick(void);
...
/**
 * @brief 是否显示命令调用函数返回�?
 *        使能此宏，则每次调用shell命令之后会以整形和十六进制的方式打印函数的返回�?
 */
#define     SHELL_DISPLAY_RETURN        0

/**
 * @brief 是否使用shell参数自动解析
 *        使能此宏以支持常规C函数形式的命令，shell会自动转换参�?
 *        关闭此宏则支持main函数形式的命令，需要自行在函数中处理参�?
 */
...
/**
 * @brief shell命令参数最大数�?
 *        包含命令名在内，超过8个参数并且使用了参数自动转换的情况下，需要修改源�?
 */
 ...
 */
#define     SHELL_GET_TICK()            get_systick()
 ...
 * @brief shell用户密码
 */
#define     SHELL_USER_PASSWORD         "ghtech"
